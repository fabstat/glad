...
def predict(device: str):
  ...
  simulator = _get_simulator(metadata, FLAGS.noise_std, FLAGS.noise_std, n_features, device)
...
def train(rank, flags, world_size, device):
  ...
  if device == torch.device("cuda"):
    serial_simulator = _get_simulator(metadata, flags["noise_std"], flags["noise_std"], n_features, rank)
  ...
  try:
    ...
    while not_reached_nsteps:
      ...
      for example in dl:  # ((position, particle_type, material_property, n_particles_per_example), labels) are in dl
        ...
        # loss = criterion(pred_acc, target_acc)
...
def _get_simulator(
  ...
  if "nnode_in" in metadata and "nedge_in" in metadata:
    ...
    nnode_in = nnode_in + 1 if n_features == 3 else nnode_in
    # nnode_in = 21 if metadata['dim'] == 3 else 16
    nedge_in = 2*metadata['dim'] #+ 1
